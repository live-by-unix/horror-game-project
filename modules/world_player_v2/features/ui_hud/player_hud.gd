...
func _ready() -> void:
	...
	var kill_player_btn = game_menu.find_child("KillPlayerButton", true, false)
	if kill_player_btn:
...
func _on_kill_player_pressed() -> void:
	print("[HUD] Kill player button pressed")
	if has_node("/root/PlayerStats"):
		PlayerStats.debug_kill_player()
...
func _on_player_died() -> void:
	print("[HUD] Player died - hiding HUD")
	# Hide the HUD when player dies
	visible = false

func show_hud() -> void:
	# Show the HUD (called after restart)
	visible = true
