...
## Handle player death - disable controls
func _on_player_died() -> void:
	print("[Player] Player died - disabling controls")
	...
	if camera_feature:
		camera_feature.set_process_input(false)
	
	# Disable combat
	if combat_feature:
		combat_feature.set_process(false)
		combat_feature.set_physics_process(false)
	
	# Disable interaction
	var interaction = get_node_or_null("Components/Interaction")
	if interaction:
		interaction.set_process(false)
		interaction.set_physics_process(false)
	
	print("[Player] All controls disabled due to death")
